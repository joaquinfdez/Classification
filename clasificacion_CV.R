...
imprime.csv <-function(m_train, accuracy, precision, recall, m_test, accuracy_test, AUC_test, herramienta, tema, algoritmo)
{
  ...
  write.table(csv, file = toString(paste("./data/result/", herramienta, tema, algoritmo,".csv", sep = " ")), row.names = FALSE, sep = ",")  
  
  
}


imprime.txt <- function(ruta, tema, algoritmo, m_train, accucacy, precision, recall, fscore, gmeasure,
...
{
  # h = 2
  for(tema in levels(dataset$Corpus))
  {
  print(c("Tema: ", tema, " con herramienta: ", herramientas[h]))
    ...
    index <- createDataPartition(Messages$Sentiment, p=0.8, list=FALSE)
    
    # Subset training set with index
    # Subset test set with index
    messages.training <- Messages[index,]
    messages.test <- Messages[-index,]
    ...
    for(t in 1:length(tecnicas))
    {
      ...
      # *********************************************************************************
      ruta <- toString(paste("./data/result/", herramientas[h], tema, algoritmo,".txt", sep = " "))
      ...
      # write.table(prediccion_probabilidad, file = toString(paste("./data/ensemble/probabilidades", herramientas[h], tema, algoritmo,".csv", sep = " ")), row.names = FALSE, sep = ",")  
      ...
      matriz_test = confusionMatrix( predictions, messages.test[,ncol(Messages)])
      ...
      #   Guardamos datos en formato TXT del aprendizaje del algoritmo
      # *********************************************************************************
      imprime.txt(ruta, tema, algoritmo, m_train, accucacy, precision, recall, fscore, gmeasure,
                  time2, time1, matriz_test, accuracy_test, AUC_test)
      
      # *********************************************************************************
      #   Guardamos datos en formato CSV del aprendizaje del algoritmo
      # *********************************************************************************
      imprime.csv(m_train, accuracy, precision, recall, m_test, accuracy_test, AUC_test, herramientas[h], tema, algoritmo)
    }
  }
}
